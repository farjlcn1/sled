...
import { importDevicesXlsx } from "./actions";

export function ImportDevicesForm() {
  const [state, formAction, pending] = useActionState(importDevicesXlsx, undefined);

  return (
    <form action={formAction} className="space-y-3 rounded-md border border-gray-200 p-4 dark:border-gray-700">
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Uvoz naprav iz XLSX</label>
        <input
          type="file"
          name="file"
          accept=".xlsx"
        ...
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
          Stolpci (poljuben vrstni red): Znamka, Model, IMEI, SRNO, Podjetje, Opomba, SIM
        </p>
      </div>
      <button
        type="submit"
      ...
      {state?.created !== undefined && (
        <div className="space-y-1 text-sm">
          <p className="text-green-700 dark:text-green-400">Uvoženih naprav: {state.created}</p>
          {state.errors && (
