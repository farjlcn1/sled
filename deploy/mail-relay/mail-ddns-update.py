#!/usr/bin/env python3
"""Posodobi DNS A-zapis mail.farjolcn.com na Cloudflaru, ce se je javni IP spremenil.

SPF zapis za farjolcn.com se sklicuje na "a:mail.farjolcn.com" namesto na trd IP, ker domaca
linija nima zagotovljeno stalnega naslova -- ta A-zapis mora torej ostati tocen, da SPF ne
razpade tiho ob spremembi IP-ja. Teče periodično prek mail-ddns.timer -- ni trajen proces, zato
ni notranje zanke/retry logike; naslednji tik timerja pokrije morebiten prehoden izpad.

Locena datoteka od deploy/ddns/ddns-update.py (gps.farjolcn.com) -- namenoma ne deljena/ne
parametrizirana s tistim skriptom, da sprememba tukaj ne more po nesreci vplivati nanj.
"""
import json
import logging
import os
...
def main() -> None:
    ...
    # proxied=False je nujen -- mail strežniki preverjajo PTR/rDNS glede na dejanski izvorni IP,
    # Cloudflarov proxy pa bi ga zamenjal s svojim robnim IP-jem.
    if not records:
        cf_request(
            "POST",
